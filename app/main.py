...
app=FastAPI(title=settings.app_name,version='0.13.4',lifespan=lifespan);app.include_router(auth_router);app.include_router(predictions_router);app.include_router(leagues_router);app.include_router(oracle_router);app.include_router(tournament_predictions_router);app.include_router(team_logos_router);app.mount('/static',StaticFiles(directory=STATIC_DIR),name='static')
...
async def team_logo_proxy(team_id:int):
 ...
 try:
  async with httpx.AsyncClient(timeout=10.0,follow_redirects=True) as client:r=await client.get(url)
  if r.status_code!=200 or not r.content:raise HTTPException(404,'Team logo not found')
  ctype=r.headers.get('content-type','image/png')
  return Response(content=r.content,media_type=ctype,headers={'Cache-Control':'public, max-age=604800, immutable'})
 except HTTPException:raise
 except Exception:raise HTTPException(502,'Team logo unavailable')
...
def serialize_match(m,h,a,logos=None):
 logos=logos or {};c=classify_ucl_round(m.season,m.kickoff_at) if m.provider=='sstats' else None;r=m.round_name or (c['round_label'] if c else None);return {'id':m.id,'provider':m.provider,'provider_id':m.provider_id,'season':m.season,'round':round_name_ru(r),'kickoff_at':m.kickoff_at,'status':m.status_short,'status_source':m.status_long,'elapsed':m.elapsed,'home':{'id':h.id,'provider':h.provider,'provider_id':h.provider_id,'name':team_name_ru(h.name),'name_original':h.name,'code':h.code,'logo':f'/api/team-logo/db/{h.id}','goals':m.home_goals},'away':{'id':a.id,'provider_id':a.provider_id,'provider':a.provider,'name':team_name_ru(a.name),'name_original':a.name,'code':a.code,'logo':f'/api/team-logo/db/{a.id}','goals':m.away_goals}}
def _first_item(p):
