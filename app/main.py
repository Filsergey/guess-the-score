...
app=FastAPI(title=settings.app_name,version='0.11.1',lifespan=lifespan);app.include_router(auth_router);app.include_router(predictions_router);app.include_router(leagues_router);app.include_router(oracle_router);app.mount('/static',StaticFiles(directory=STATIC_DIR),name='static')
@app.get('/',include_in_schema=False,response_class=HTMLResponse)
async def mini_app():
 html=(STATIC_DIR/'index.html').read_text(encoding='utf-8');return HTMLResponse(html.replace('</body>','<script src="/static/oracle-ui.js?v=4"></script><script src="/static/oracle-leaderboard.js?v=1"></script></body>'))
