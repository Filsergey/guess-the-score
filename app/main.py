...
def serialize_sstats_details(data: dict, glicko: dict | None = None) -> dict:
    ...
    statistics = {
        name: {"home": _v(data, name + "Home"), "away": _v(data, name + "Away")}
        for name in stat_names
    }
    glicko = glicko or {}
    return {
        "flash_id": _v(data, "FlashId"),
        "season_uid": _v(data, "SeasonUid"),
        "venue": {
            "id": _v(data, "VenueId"),
            "name": _v(data, "VenueName"),
            "city": _v(data, "VenueCity"),
            "address": _v(data, "VenueAddress"),
        },
        "coaches": {
            "home": _v(data, "HomeTeamCoachName"),
            "away": _v(data, "AwayTeamCoachName"),
        },
        "score": {
            "ht": {"home": _v(data, "ScoreHomeHT"), "away": _v(data, "ScoreAwayHT")},
        ...
        "odds": {
            "home": _v(data, "Winner1"),
            "draw": _v(data, "WinnerX"),
            "away": _v(data, "Winner2"),
        },
        "model": {
            "rating": {
                "home": _v(data, "GlickoRatingHome") or _v(glicko, "RatingHome"),
                "away": _v(data, "GlickoRatingAway") or _v(glicko, "RatingAway"),
            },
            "win_probability": {
                "home": _v(data, "GlickoWinProbHome") or _v(glicko, "WinProbHome"),
                "away": _v(data, "GlickoWinProbAway") or _v(glicko, "WinProbAway"),
            },
            "xg": {
                "home": _v(data, "GlickoXgHome") or _v(glicko, "XgHome"),
                "away": _v(data, "GlickoXgAway") or _v(glicko, "XgAway"),
            },
            "odds_xg": {"home": _v(data, "OddsXgHome"), "away": _v(data, "OddsXgAway")},
        },
        "statistics": statistics,
        "coverage": {
            "players": _v(data, "CoverageSeasonPlayers"),
            "events": _v(data, "CoverageSeasonEvents"),
            "lineups": _v(data, "CoverageSeasonLineups"),
            "fixture_statistics": _v(data, "CoverageSeasonStatisticsFixtures"),
            "player_statistics": _v(data, "CoverageSeasonStatisticsPlayers"),
            "standings": _v(data, "CoverageSeasonStandings"),
...
async def match_rich_detail(match_id: int, db: AsyncSession = Depends(get_db)) -> dict:
    ...
    if match.provider != "sstats":
        return {**base, "details_available": False, "details_source": None, "details": None}
    ...
    try:
        query_data = _first_item(await provider.query_game_details(match.provider_id))
    except Exception as exc:
        errors.append(f"query:{type(exc).__name__}")

    if not query_data:
        try:
            game_data = _first_item(await provider.get_game(match.provider_id))
        except Exception as exc:
            errors.append(f"game:{type(exc).__name__}")

    try:
        glicko_data = _first_item(await provider.get_glicko(match.provider_id))
    except Exception as exc:
        errors.append(f"glicko:{type(exc).__name__}")

    data = _merge_non_empty(query_data, game_data) if (query_data or game_data) else {}
    if not data and not glicko_data:
        return {
            **base,
    ...
    return {
        **base,
        ...
        "details_errors": errors,
        "details": serialize_sstats_details(data, glicko_data),
    }
