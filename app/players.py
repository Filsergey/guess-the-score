...
def _player_name(row:dict)->str|None:
    ...
    first=str(_pick(row,"firstName","FirstName",default="") or "").strip();last=str(_pick(row,"lastName","LastName",default="") or "").strip()
...
def _apply_player_row(player:Player,row:dict,team:Team|None,season:int|None)->None:
    ...
    player.team_provider_id=team.provider_id if team else player.team_provider_id;player.team_name=team.name if team else player.team_name
    player.position=_position(row) or player.position;player.shirt_number=_pick(row,"shirtNumber","ShirtNumber","number","Number",default=player.shirt_number);player.nationality=_nationality(_pick(row,"nationality","Nationality",default=player.nationality))
...
async def _refresh_player_detail(db:AsyncSession,player:Player)->None:
    ...
    try:
        ...
        team=await db.scalar(select(Team).where(Team.provider=="sstats",Team.provider_id==player.team_provider_id)) if player.team_provider_id else None
        _apply_player_row(player,row,team,player.season);await db.commit()
...
async def list_players(q:str|None=Query(default=None,max_length=100),popular:bool=False,limit:int=Query(default=30,ge=1,le=100),user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db)):
    ...
    if not rows:
        await ensure_sstats_player_catalog(db);rows=await _player_rows(db,q,popular,limit)
