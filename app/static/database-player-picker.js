...
// Fallback team selector for Telegram WebView. Some WebView builds lose the
// per-item onclick handlers when the bottom sheet is re-rendered. Handle team
// items at document capture level so a tap always commits the selection.
function selectTeamItem(item){
 var picker=item.closest('.tp-picker');if(!picker)return;
 var hidden=picker.querySelector('input[type="hidden"]'),control=picker.querySelector('.tp-control'),menu=picker.querySelector('.tp-menu');if(!hidden||!control)return;
 var label=(item.querySelector('span')||item).textContent.trim();
 hidden.value=label;
 var img=item.querySelector('img.tp-logo');
 control.innerHTML=(img?'<img class="tp-logo" src="'+esc(img.getAttribute('src')||'')+'" alt="">':'<span class="tp-crest-fallback">⚽</span>')+'<span>'+esc(label)+'</span><span class="tp-chevron">⌄</span>';
...
document.addEventListener('click',function(e){
 var teamItem=e.target.closest&&e.target.closest('.tp-menu [data-team-index]');
 if(teamItem){e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();selectTeamItem(teamItem);return}
 var c=e.target.closest&&e.target.closest('.tp-player-control');if(!c)return;e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openPicker(c)
},true);
