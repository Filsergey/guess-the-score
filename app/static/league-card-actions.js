...
#leaguesView .league-card.has-delete .league-emblem{flex:0 0 48px;width:48px;height:48px;margin-right:10px}
...
#leaguesView .league-card.has-delete .league-name{display:block;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:14px;line-height:17px}
#leaguesView .league-card.has-delete .league-role{margin-left:5px;flex:0 0 auto}
...
#leaguesView .league-edit{position:absolute;right:49px;top:50%;transform:translateY(-50%);height:27px;padding:0 7px;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#9fd3ff;background:rgba(25,88,137,.16);border:1px solid rgba(63,143,203,.32);font-size:8px;font-weight:850;z-index:3;white-space:nowrap}
...
#leaguesView .gts-tournament-line{font-size:9px;color:#a9c4dd;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
#leaguesView .gts-members-line{font-size:10px;font-weight:850;color:#43ff78;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}
...
function decorate(){const cards=[...document.querySelectorAll('#leaguesView .league-card')],leagues=window.getLeagues?.()||[];cards.forEach((card,index)=>{const id=leagueIdFromCard(card,index),league=leagues.find(x=>Number(x.id)===id)||leagues[index];if(!id)return;card.dataset.leagueId=String(id);splitMeta(card);if(canManage(league)&&!card.querySelector('.league-edit')){const btn=document.createElement('span');btn.className='league-edit';btn.setAttribute('role','button');btn.textContent='Изменить';btn.onclick=e=>{e.preventDefault();e.stopPropagation();editLeagueSettings?.(id)};const del=card.querySelector('.league-delete');if(del)card.insertBefore(btn,del);else card.appendChild(btn)}});const selected=Number(window.GTS?.leagueId||0);if(selected)markSelected(selected)}
