...
function loadTelegramLoginSdk(){if(window.Telegram?.Login)return Promise.resolve();return new Promise((resolve,reject)=>{const old=document.querySelector('script[data-gts-telegram-login-sdk]');if(old){old.addEventListener('load',()=>resolve(),{once:true});old.addEventListener('error',()=>reject(new Error('Не удалось загрузить Telegram Login')),{once:true});return}const s=document.createElement('script');s.src='https://oauth.telegram.org/js/telegram-login.js?3';s.async=true;s.dataset.gtsTelegramLoginSdk='1';s.onload=()=>resolve();s.onerror=()=>reject(new Error('Не удалось загрузить Telegram Login'));document.head.appendChild(s)})}
function browserLogin(status,state){
 ...
 browserLoginPromise=new Promise((resolve,reject)=>{
  ...
  btn.onclick=async()=>{btn.disabled=true;btn.textContent='Открываем Telegram…';err.textContent='';try{await loadTelegramLoginSdk();if(!window.Telegram?.Login)throw new Error('Telegram Login недоступен');window.Telegram.Login.auth({client_id:Number(status.client_id),scope:['profile'],lang:'ru'},async result=>{try{if(!result||result.error||!result.id_token)throw new Error(result?.error||'Вход не завершён');const r=await fetch('/api/auth/web/id-token',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({id_token:result.id_token})});let d={};try{d=await r.json()}catch{}if(!r.ok)throw new Error(d.detail||'Ошибка входа');state.token=d.access_token;state.me=d.user;root.remove();style.remove();browserLoginPromise=null;resolve(state.me)}catch(e){err.textContent=e.message||'Не удалось войти';btn.disabled=false;btn.textContent='Войти через Telegram'}})}catch(e){if(status.fallback_login_url){location.assign(status.fallback_login_url);return}err.textContent=e.message||'Не удалось открыть Telegram';btn.disabled=false;btn.textContent='Войти через Telegram'}};
 });
...
const state={
 ...
 async login(){
  ...
  if(!tg?.initData){
   ...
   if(status?.configured&&status?.client_id)return browserLogin(status,this);
...
if(!document.querySelector('script[data-gts-pwa]')){const s=document.createElement('script');s.src='/static/pwa.js?v=2';s.dataset.gtsPwa='1';document.head.appendChild(s)}
