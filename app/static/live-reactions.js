...
document.addEventListener('gts:matches-updated',ev=>{const matches=ev.detail?.matches||[];if(!previous.size){previous=new Map(matches.map(m=>[Number(m.id),structuredClone(m)]));return}let anyFinished=false;for(const m of matches){const id=Number(m.id),old=previous.get(id);if(old){const text=teamGoalText(old,m);if(text){flash(id);showGoal(text);document.dispatchEvent(new CustomEvent('gts:goal',{detail:{before:old,match:m}}))}if(finished(old,m)){anyFinished=true;document.dispatchEvent(new CustomEvent('gts:match-finished',{detail:{before:old,match:m}}))}}previous.set(id,structuredClone(m))}if(anyFinished)refreshCompetitionViews()});
})();