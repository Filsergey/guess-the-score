...
async def save_prediction(match_id: int, body: PredictionInput, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)) -> dict:
    ...
    if now >= match.kickoff_at: raise HTTPException(status_code=409, detail="Prediction is locked because the match has started")
    if match.kickoff_at < user.registered_at: raise HTTPException(status_code=409, detail="Match is not eligible for this user")
    prediction = await db.scalar(select(Prediction).where(Prediction.user_id == user.id, Prediction.match_id == match_id))
    if prediction is None:
        prediction = Prediction(user_id=user.id, match_id=match_id, home_score=body.home_score, away_score=body.away_score, created_at=now, updated_at=now); db.add(prediction)
    else:
        prediction.home_score=body.home_score; prediction.away_score=body.away_score; prediction.updated_at=now
...
async def match_prediction_participants(match_id:int,user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db))->dict:
    ...
    for prediction,participant in rows:
        mine=participant.id==user.id
        item={"user_id":participant.id,"display_name":participant.display_name,"username":participant.username,"avatar_url":participant.avatar_url,"has_prediction":True,"is_mine":mine}
        # Before kickoff other users' score fields never leave the backend.
        if started or mine:item["prediction"]={"home_score":prediction.home_score,"away_score":prediction.away_score,"points":prediction_points(prediction,match)}
