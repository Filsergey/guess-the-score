...
async def save_prediction(
    ...
    prediction = await db.scalar(
        select(Prediction).where(Prediction.user_id == user.id, Prediction.match_id == match_id)
    )
...
async def my_prediction(
    ...
    return {
        ...
        "prediction": serialize_prediction(prediction, match) if prediction else None,
    }
