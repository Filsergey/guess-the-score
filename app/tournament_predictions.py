...
async def _competition_teams(db,provider,season):
 ...
 if not ids:return []
 teams=(await db.execute(select(Team).where(Team.id.in_(ids)).order_by(Team.name))).scalars().all();return [{'id':t.id,'provider_id':t.provider_id,'name':t.name,'display_name':team_name_ru(t.name),'logo':t.logo_url} for t in teams]
def _player_rows(payload):
 ...
 for x in data if isinstance(data,list) else []:
  ...
  seen.add(key);team=x.get('teamName',x.get('TeamName'));out.append({'id':pid,'name':str(name),'team':team_name_ru(team),'photo':x.get('photo',x.get('Photo'))})
 return out[:20]
