...
def _apply(match: Match, row: dict) -> bool:
    short, long_name = _normalize_status(
        _pick(row, "status", "Status"),
        _pick(row, "statusName", "StatusName"),
    )
    home = _int(_pick(row, "scoreHome", "ScoreHome", "homeResult", "HomeResult"))
    away = _int(_pick(row, "scoreAway", "ScoreAway", "awayResult", "AwayResult"))
    ...
    rows = (
        await db.execute(
            ...
            .where(
                ...
                UserLeague.tournament_provider == "sstats",
                UserLeague.tournament_id.is_not(None),
            )
            .distinct()
        )
    ).all()
    scopes = {
        (int(tid), int(provider_id), int(season))
        for tid, provider_id, season in rows
        if tid is not None and provider_id is not None and season is not None
    }
    if not scopes:
        ucl = await db.scalar(
            select(Tournament).where(
                Tournament.provider == "sstats",
                Tournament.provider_id == 2,
            )
        )
        if ucl is not None:
            scopes.add((ucl.id, 2, int(fallback_season)))
...
async def sync_sstats_live_matches(db: AsyncSession, season: int) -> dict:
    ...
    resolving /Leagues on every 30-second tick. Games/list?Live=true is the primary
    source. Individual game probes are only a fallback for provider lag.

    Near-kickoff games are deliberately probed before older stale fixtures. This is
    important when several Champions League matches start at the same time: an old
    match whose final status has not reached our DB must never consume the small
    fallback budget and leave a newly started match displayed as "upcoming".
    """
    provider = SStatsProvider()
    scopes = await _competition_scopes(db, season)
    ...
    finished_captured = 0
    kickoff_checked = 0
    priority_kickoff_checked = 0
    older_kickoff_checked = 0
    ...
    for tournament_id, league_id, scope_season in scopes:
        try:
            payload = await provider.get_games(
                league_id=league_id,
                year=scope_season,
                live=True,
                limit=1000,
            ...
            live_ids.update(scope_live_ids)
            live_received += len(rows)
                ...
                (
                    await db.execute(
                        select(Match).where(
                            ...
                            Match.provider_id.in_(incoming),
                        )
                    )
                ).scalars().all()
                if incoming
            ...
            live_matched += len(matches)
            scope_changed = 0
            for row in rows:
                match = by_provider_id.get(str(_pick(row, "id", "Id")))
                if match:
                    did = _apply(match, row)
                    changed += int(did)
                    scope_changed += int(did)
    ...
    # A DB-live match that vanished from Live=true may just have finished.
    for match in db_live[:4]:
        if str(match.provider_id) in live_ids:
            continue
        try:
            row = _first(await provider.get_game(int(match.provider_id)))
            stale_checked += 1
            if row:
                was_live = match.status_short in LIVE_MATCH_STATUSES
                did = _apply(match, row)
                changed += int(did)
                if was_live and match.status_short in FINAL_MATCH_STATUSES:
                    finished_captured += 1
        except Exception as exc:
            if len(errors) < 12:
                errors.append(
    ...
    checked_provider_ids: set[str] = set()

    # Priority fallback: probe every match around the current kickoff window first.
    # A UCL slate commonly has four simultaneous starts, so a limit of 8 leaves room
    # for another competition without allowing unbounded request growth.
    if scope_tournament_ids:
        priority = (
            (
                await db.execute(
        ...
        for match in priority:
            provider_id = str(match.provider_id)
            checked_provider_ids.add(provider_id)
            if provider_id in live_ids:
                continue
            try:
                row = _first(await provider.get_game(int(match.provider_id)))
                kickoff_checked += 1
                priority_kickoff_checked += 1
                if row:
                    changed += int(_apply(match, row))
            except Exception as exc:
                if len(errors) < 12:
                    errors.append(
                        {
                            ...
                            "stage": "priority-kickoff-window",
                            "error": type(exc).__name__,
                        }
                    )

        # Small secondary budget for older non-final fixtures. This keeps delayed
        # final statuses moving without letting them starve newly kicked-off games.
        older = (
            (
                await db.execute(
                    select(Match)
        ...
        for match in older:
            provider_id = str(match.provider_id)
            if provider_id in live_ids or provider_id in checked_provider_ids:
                continue
            try:
                row = _first(await provider.get_game(int(match.provider_id)))
                kickoff_checked += 1
                older_kickoff_checked += 1
                if row:
                    changed += int(_apply(match, row))
            except Exception as exc:
                if len(errors) < 12:
    ...
    await db.commit()
    try:
        test_seed = await ensure_test_reaction_participant(db)
    except Exception as exc:
        test_seed = {"seeded": False, "error": type(exc).__name__}
    try:
    ...
    return {
        ...
        "competitions": len(scopes),
        "scopes": scope_results,
        "live_received": live_received,
        "live_matched": live_matched,
        "db_live_before": len(db_live),
        "stale_live_checked": stale_checked,
        "kickoff_window_checked": kickoff_checked,
        "priority_kickoff_checked": priority_kickoff_checked,
        "older_kickoff_checked": older_kickoff_checked,
        "finished_captured": finished_captured,
        "changed": changed,
