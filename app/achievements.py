...
router=APIRouter(prefix='/api/leagues',tags=['achievements'])


def _streak(values,needle):
    best=cur=0
    for v in values:
        cur=cur+1 if v==needle else 0;best=max(best,cur)
    return best
...
async def achievements(league_id:int,user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db)):
    ...
    matches=(await db.execute(select(Match).where(Match.provider==league.tournament_provider,Match.season==league.tournament_season,Match.status_short.in_(tuple(FINAL_MATCH_STATUSES)),Match.home_goals.is_not(None),Match.away_goals.is_not(None)).order_by(Match.kickoff_at))).scalars().all();ids=[m.id for m in matches];by_id={m.id:m for m in matches}
    all_preds=(await db.execute(select(Prediction).where(Prediction.match_id.in_(ids)))).scalars().all() if ids else [];preds={(p.user_id,p.match_id):p for p in all_preds}
    oracle_rows=(await db.execute(select(OraclePrediction).where(OraclePrediction.match_id.in_(ids)))).scalars().all() if ids and league.include_oracle else [];oracle={p.match_id:p for p in oracle_rows}
    rows=[]
    for _,u in members:
        ...
        for m in eligible:
            ...
            points.append(pts);exacts+=pts==3;outcomes+=pts==1
            os=_oracle_score(oracle.get(m.id),m) if league.include_oracle else None
        ...
        exact_streak=_streak(points,3);hit_streak=max((_streak(points[:i]+[],points[i-1]) for i in range(1,len(points)+1)),default=0) if False else 0
        cur=best_hit=0
    ...
    rows.sort(key=lambda x:(-len(x['achievements']),-x['exacts'],-x['oracle_wins'],x['display_name'].casefold()))
    mine=next((x for x in rows if x['is_me']),None)
    return {'league_id':league_id,'count':len(rows),'mine':mine,'response':rows}
